---
id: "session-008"
type: "session"
name: "더 월드 41 룩스테라-재료 수급로 참변 수색"
summary: "지난 날, 유적 의뢰를 마치고 돌아온 일행은 충격적인 소식을 듣게 됩니다."
canon_status: "canon"
confidence: "medium"
source_type: "rp_log"
sources:
  - "source/rp-logs/luxterra/session-008-41.md"
tags:
  - "session"
  - "luxterra"
first_seen: "session-008"
last_updated: "2026-05-24"
date_played: ""
related_characters:
  - "char-h1n38m25"
  - "char-hutxq0x"
  - "char-hvqt0o9"
  - "char-hp"
  - "char-hz6cjul"
  - "char-hbfjmmh"
  - "char-h220ux9"
  - "char-x"
  - "char-h1xf9zef"
  - "char-h1epqndl"
  - "char-h17qcav"
  - "char-mugfist-knuckles"
related_locations:
  - "loc-origin"
  - "loc-ruins"
  - "loc-forest"
  - "loc-yggdrasil-gap"
related_factions:
  []
related_events:
  - "event-session-008"
---
# 더 월드 41 룩스테라-재료 수급로 참변 수색

## 요약

지난 날, 유적 의뢰를 마치고 돌아온 일행은 충격적인 소식을 듣게 됩니다.

## 주요 장면

- GM: 지난 날, 유적 의뢰를 마치고 돌아온 일행은 충격적인 소식을 듣게 됩니다.
- 크리스 반데드: 이 일에 대해 크리스 반데드가 짚히는 점이 있다며 회관에서 정보를 뱉고 있습니다.
- 매그너스: (그게 왜 진조잘못이더냐?)
- Gamemaster: 매그너스 heals 1 HP
- 매그너스: Long Rest (8 Hours, New Day)

## 새로 등장한 정보

### 캐릭터

- 매그너스 (char-h1n38m25)
- 순대 (char-hutxq0x)
- 안톤 (char-hvqt0o9)
- HP 업데이트 됨 두 배 적용 취소 (char-hp)
- 이브 (char-hz6cjul)
- 카이젤린 (char-hbfjmmh)
- 흰 고릴라 투척병 (char-h220ux9)
- x (char-x)
- 빨간 이빨 부족장 카사카르 (char-h1xf9zef)
- 마르도스 (char-h1epqndl)
- 하얀 바람 부족장 고룬노바 (char-h17qcav)
- Mugfist Knuckles (char-mugfist-knuckles)

### 지역

- 오리진 (loc-origin)
- 유적 (loc-ruins)
- 숲 (loc-forest)
- 세계의 틈새 (loc-yggdrasil-gap)

### 팩션

- 확인 필요

### 사건

- 더 월드 41 룩스테라-재료 수급로 참변 수색 (event-session-008)

## 확정된 정보

- 이 문서는 공개 로그를 기반으로 자동 생성한 1차 정리본입니다.
- 명확한 원문 근거가 부족한 해석은 확정 정보로 적지 않았습니다.

## 소문 / 불확실한 정보

- 장면의 원인, 배후, 비공개 동기처럼 로그 밖 추론이 필요한 정보는 아직 확정하지 않았습니다.
- 자동 추출 과정에서 누락된 인물과 장소가 있을 수 있습니다.

## 관계 변화

| 대상 A | 대상 B | 변화 |
|---|---|---|
| 확인 필요 | 확인 필요 | 세부 관계는 2차 정리에서 로그 대조가 필요합니다. |

## 다음 세션으로 이어지는 요소

- 미해결 갈등, 이동 예정지, 추적 중인 단서는 2차 정리에서 캠페인별로 검수합니다.

## 원문 출처

- source/rp-logs/luxterra/session-008-41.md
