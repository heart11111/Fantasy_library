---
id: "session-013"
type: "session"
name: "더월드 35 룩스테라-모험의 서막"
summary: "붉은 닻 도적단 소탕 후 촌장 아카이럼과 이브를 구출한다. 그레이브록 마을에서 정제사 에리나를 만나 부정한 핵이 아이템이 되는 과정을 목격하고, 다음날 아침 메라크와 라비니아가 도착해 파티가 재결집한다."
canon_status: "canon"
confidence: "medium"
source_type: "rp_log"
sources:
  - "source/rp-logs/luxterra/session-013-35-txt.md"
tags:
  - "session"
  - "luxterra"
first_seen: "session-013"
last_updated: "2026-05-24"
date_played: ""
related_characters:
  - "char-merak-altemecia"
  - "char-lavinia"
  - "char-hdslylj"
  - "char-lunasha"
  - "char-eve"
  - "char-hekatriel"
  - "char-h13qb48p"
  - "char-h1skfz84"
  - "char-hd11vjr"
  - "char-h1np88gm"
  - "char-h1dmj0xh"
  - "char-h19p0ig5"
related_locations:
  - "loc-luxterra"
  - "loc-origin"
  - "loc-ruins"
  - "loc-city"
  - "loc-yggdrasil-gap"
related_factions:
  - "faction-guard"
  - "faction-red-anchor"
  - "faction-bandits"
  - "faction-skeletons"
related_events:
  - "event-session-013"
---
# 더월드 35 룩스테라-모험의 서막

## 요약

지금까지의 룩스테라 이야기:

## 주요 장면

- Gamemaster: 지금까지의 룩스테라 이야기:
- Gamemaster: 붉은 닻 도적단이 숨겨놓은 통로에 들어가 그 안에 있는 것들을 모조리 쓸어버린 여러분은 촌장 아카이럼과 이브를 구출해냈지만 아직 저 깊은 곳에 있는 위험을 남겨두고 휴식을 취하기 위해 그레이브록 마을로 왔습니다.
- 매그너스: (엇 나 그 이야기에는 없었는데
- Gamemaster: 그레이브록 마을에서 정제사 에리나를 만나 부정한 핵이 아이템이 되는 과정을 보았고 이런 저런 이야기들을 하며 휴식을 취합니다.
- 매그너스: (그럼 처음부터 매그너스인가

## 새로 등장한 정보

### 캐릭터

- 메라크 알테메기아 (char-merak-altemecia)
- 라비니아 (char-lavinia)
- 지크프리드 (char-hdslylj)
- 루나샤 (char-lunasha)
- 이브 (char-eve)
- 헤카트리엘 (char-hekatriel)
- 흑마석 거인 (char-h13qb48p)
- 촉수 감염체 (char-h1skfz84)
- 맹독 돌연변이 군주 (char-hd11vjr)
- 촌장 아카이럼 (char-h1np88gm)
- 오리진 총책임자 듀란 (char-h1dmj0xh)
- 오리진 부관 카밀론 (char-h19p0ig5)

### 지역

- 룩스테라 (loc-luxterra)
- 오리진 (loc-origin)
- 유적 (loc-ruins)
- 도시 (loc-city)
- 세계의 틈새 (loc-yggdrasil-gap)

### 팩션

- 수비대 (faction-guard)
- 붉은 닻 (faction-red-anchor)
- 산적 (faction-bandits)
- 해골 (faction-skeletons)

### 사건

- 더월드 35 룩스테라-모험의 서막 (event-session-013)

## 확정된 정보

- 이 문서는 공개 로그를 기반으로 자동 생성한 1차 정리본입니다.
- 명확한 원문 근거가 부족한 해석은 확정 정보로 적지 않았습니다.

## 소문 / 불확실한 정보

- 장면의 원인, 배후, 비공개 동기처럼 로그 밖 추론이 필요한 정보는 아직 확정하지 않았습니다.
- 자동 추출 과정에서 누락된 인물과 장소가 있을 수 있습니다.

## 관계 변화

| 대상 A | 대상 B | 변화 |
|---|---|---|
| 확인 필요 | 확인 필요 | 세부 관계는 2차 정리에서 로그 대조가 필요합니다. |

## 다음 세션으로 이어지는 요소

- 미해결 갈등, 이동 예정지, 추적 중인 단서는 2차 정리에서 캠페인별로 검수합니다.

## 원문 출처

- source/rp-logs/luxterra/session-013-35-txt.md
